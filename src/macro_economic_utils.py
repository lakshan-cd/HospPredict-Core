"""
Utility functions for Macro-Economic Module
...
class MacroEconomicTrainer:
    """Handles model training for macro-economic analysis"""
    
    def __init__(self):
        self.model = None
        self.history = None
    
    def build_model(self, input_shape_stock: Tuple, input_shape_macro: Tuple) -> Model:
        """
        Build dual-input LSTM model
        
        Args:
            input_shape_stock: Shape of stock input (timesteps, features)
            input_shape_macro: Shape of macro input (timesteps, features)
            
        Returns:
            Compiled Keras model
        """
        # Stock input branch
        input_stock = Input(shape=input_shape_stock, name="Stock_Input")
        x1 = LSTM(64, return_sequences=False)(input_stock)
        
        # Macro input branch
        input_macro = Input(shape=input_shape_macro, name="Macro_Input")
    ...
    def train_model(self, company_name: str, epochs: int = 50, batch_size: int = 32) -> Dict[str, Any]:
        ...
        try:
            ...
            self.model = self.build_model(
                input_shape_stock=(X_stock.shape[1], 1),
                input_shape_macro=(X_macro.shape[1], X_macro.shape[2])
            )
            ...
            # Evaluate model
            y_pred = self.model.predict([X_stock_test, X_macro_test])
...
class MacroEconomicPredictor:
    ...
    def load_model(self, company_name: str) -> bool:
        ...
        try:
            ...
            self.models[company_name] = load_model(model_path, compile=False)
    ...
    def predict(self, company_name: str, days_ahead: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            # Make prediction
            prediction_scaled = self.models[company_name].predict([latest_stock, latest_macro])
            prediction = self.scalers[company_name]['stock'].inverse_transform(
    ...
    def evaluate_model(self, company_name: str) -> Dict[str, Any]:
        """
        Evaluate model performance for a company
        ...
        try:
            ...
            X_stock = np.load(os.path.join(SEQUENCE_DIR, f"{company_name}_X_stock.npy"))
            X_macro = np.load(os.path.join(SEQUENCE_DIR, f"{company_name}_X_macro.npy"))
            y_scaled = np.load(os.path.join(SEQUENCE_DIR, f"{company_name}_y.npy"))
            
            # Make predictions for all data
            y_pred_scaled = self.models[company_name].predict([X_stock, X_macro])
            
            # Inverse transform
            y_pred = self.scalers[company_name]['stock'].inverse_transform(
                y_pred_scaled.reshape(-1, 1)
            ).flatten()
            y_actual = self.scalers[company_name]['stock'].inverse_transform(
            ...
            raise Exception(f"Error evaluating model for {company_name}: {str(e)}")
